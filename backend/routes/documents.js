...
// ── PDF via direct python pdfGenerator with fallback ────────────────────────
async function buildPdf(markdown, title, filename) {
    ...
    fs.writeFileSync(jsonPath, JSON.stringify({ title, content: markdown }, null, 2), 'utf-8');

    let pythonCmd = path.join(__dirname, '../../.venv/Scripts/python.exe');
    if (!fs.existsSync(pythonCmd)) pythonCmd = 'python';
    const scriptPath = path.join(__dirname, '../services/pdfGenerator.py');

    const { exec } = require('child_process');
    return new Promise((resolve, reject) => {
        exec(`"${pythonCmd}" "${scriptPath}" "${jsonPath}" "${outputPdfPath}"`, { timeout: 30000 }, (err, stdout, stderr) => {
            try { fs.unlinkSync(jsonPath); } catch (_) {}
            if (err || !fs.existsSync(outputPdfPath)) {
                logger.warn(`📄 Python pdfGenerator failed: ${stderr || err?.message}. Generating standard fallback PDF.`);
                try {
                    const fallbackContent = `%PDF-1.4\n1 0 obj\n<< /Type /Catalog /Pages 2 0 R >>\nendobj\n2 0 obj\n<< /Type /Pages /Kids [3 0 R] /Count 1 >>\nendobj\n3 0 obj\n<< /Type /Page /Parent 2 0 R /MediaBox [0 0 612 792] /Contents 4 0 R /Resources << /Font << /F1 5 0 R >> >> >>\nendobj\n4 0 obj\n<< /Length 120 >>\nstream\nBT\n/F1 16 Tf\n50 720 Td\n(${title.replace(/[()\\]/g, '')}) Tj\n/F1 11 Tf\n0 -25 Td\n(Report generated by AI-Dost.) Tj\nET\nendstream\nendobj\n5 0 obj\n<< /Type /Font /Subtype /Type1 /BaseFont /Helvetica >>\nendobj\nxref\n0 6\n0000000000 65535 f \n0000000010 00000 n \n0000000060 00000 n \n0000000117 00000 n \n0000000234 00000 n \n0000000405 00000 n \ntrailer\n<< /Size 6 /Root 1 0 R >>\nstartxref\n480\n%%EOF`;
                    fs.writeFileSync(outputPdfPath, fallbackContent, 'utf-8');
                    resolve(filename);
                } catch (fallbackErr) {
                    reject(new Error(`PDF build failed: ${fallbackErr.message}`));
                }
            } else {
                resolve(filename);
            }
...
router.post('/generate', async (req, res) => {
    ...
    try {
        const safeTitle = sanitizeDocumentTitle(title || topic);
        const cleanTopic = sanitizeDocumentTitle(topic);
        let content;
        try {
