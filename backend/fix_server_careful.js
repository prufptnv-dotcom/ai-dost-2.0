...
// 2. Project Memory Endpoints
const target2Start = `// ── Project Memory endpoints (SQLite-backed) ──────────────────────────`;
const target2End = `// Find in Files - line-based search across a project's workspace (Ctrl+Shift+F)`;

const replacement2 = `// ── Project Memory endpoints (SQLite-backed) ──────────────────────────
...
} else if (content.includes(target1.replace(/\r/g, ''))) {
...
// Replace Endpoints
const idx1 = content.indexOf(target2Start);
const idx2 = content.indexOf(target2End);
if (idx1 !== -1 && idx2 !== -1) {
    const head = content.slice(0, idx1);
    // Find the end of the Find in Files endpoint which originally ends around line 763
    // It's app.get('/api/...search', (req, res) => { ... })
    // We'll replace from target2Start to the next // Chat history
    const endMarker = `// Chat history persistence endpoints`;
    const idxEnd = content.indexOf(endMarker, idx2);
    if (idxEnd !== -1) {
        const tail = content.slice(idxEnd);
        content = head + replacement2 + tail;
    } else {
        console.error('End marker not found');
    }
} else {
    console.error('Target 2 bounds not found');
}

// Replace getChatHistory / deleteChatHistory / saveChatHistory
const chatStartMarker = `// Chat history persistence endpoints (registered under both /api and /api/v1 —`;
let chatIdx = content.indexOf(chatStartMarker);
if (chatIdx === -1) {
    chatIdx = content.indexOf(`// Chat history persistence endpoints (registered under both /api and /api/v1`);
}

const chatEndMarker = `// Root redirect to frontend dev server`;
const chatEndIdx = content.indexOf(chatEndMarker, chatIdx);

const chatReplacement = `// Chat history persistence endpoints (delegated to ConversationDAO / MessageDAO with legacy fallback)
...
if (chatIdx !== -1 && chatEndIdx !== -1) {
    const head = content.slice(0, chatIdx);
    const tail = content.slice(chatEndIdx);
    content = head + chatReplacement + tail;
} else {
    console.error('Chat boundaries not found');
}

// Ensure requires are added at top
