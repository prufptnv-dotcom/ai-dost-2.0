...
function putInMainComposer(text) {
...
async function readFileDescriptor(file) {
  ...
  if (file.type === 'application/pdf' || file.name.toLowerCase().endsWith('.pdf')) {
    return { ...base, kind: 'pdf', previewUrl: URL.createObjectURL(file), dataUrl: await readDataUrl(file) };
  }
  return { ...base, kind: 'text', text: (await file.text()).slice(0, MAX_TEXT_CHARS) };
}

export default function ChatExperienceLayerV3({ onNavigate, onNewChat, onDeleteChat }) {
  ...
  useEffect(() => {
    ...
    const observer = new MutationObserver(() => {
      ...
      if (!label) return;
      const name = /search/i.test(label) ? 'Searching' : /read|analy/i.test(label) ? 'Reading' : /generat|creat|document/i.test(label) ? 'Writing' : /execut/i.test(label) ? 'Executing' : /verif/i.test(label) ? 'Verifying' : 'Thinking';
  ...
  const chooseTheme = (theme) => {
    setThemeId(theme.id);
    setThemeVariables(theme);
    try { localStorage.setItem('ai_dost_chat_theme_premium', theme.id); } catch (_) {}
  };
  ...
  const onFilesSelected = async (event) => {
    ...
    if (selected.length > room) {
      window.dispatchEvent(new CustomEvent('ai_dost_toast', { detail: { type: 'warning', message: `Only ${MAX_FILES} files can be attached at once.` } }));
    }
    const next = [];
    for (const file of chosen) {
      try { next.push(await readFileDescriptor(file)); } catch (_) { /* ignore unreadable file */ }
    }
  ...
  const continueInChat = () => {
    ...
    if (context) putInMainComposer(`Use these file-analysis results as context and continue the task:\n\n${context.slice(0, 45000)}`);
    setOpen(false);
  };
  ...
  const toggleVoice = () => {
    ...
    recognition.onresult = (event) => {
      ...
      if (text) putInMainComposer(text);
    };
    ...
    <>
      ...
      {open && <div className="fixed inset-0 z-[65] pointer-events-none">
        <div className="absolute right-5 bottom-20 pointer-events-auto w-[min(94vw,430px)] max-h-[78vh] overflow-hidden rounded-2xl border border-border bg-canvas-surface/95 backdrop-blur-xl shadow-2xl">
          <div className="flex items-center justify-between px-4 py-3 border-b border-border-subtle"><div><div className="flex items-center gap-2 text-sm font-semibold text-paper-100"><Command className="w-4 h-4 text-accent" /> Universal Chat</div><div className="text-[10px] text-ink-muted">Anything you ask stays in the main chat workflow.</div></div><button onClick={() => setOpen(false)} aria-label="Close" className="p-1.5 rounded-lg hover:bg-canvas-elevated"><X className="w-4 h-4" /></button></div>
          <div className="p-3 space-y-3 overflow-y-auto max-h-[calc(78vh-56px)]">
            <section className="rounded-xl border border-border bg-canvas-base/60 p-2"><div className="flex items-center gap-2"><Command className="w-4 h-4 text-ink-muted" /><input value={query} onChange={(e) => { setQuery(e.target.value); setShowCommands(true); }} onFocus={() => setShowCommands(true)} onKeyDown={(e) => { if (e.key === 'Enter' && filteredCommands[0]) chooseCommand(filteredCommands[0][2]); }} placeholder="open projects · new chat · delete chat…" aria-label="Universal chat command" className="flex-1 bg-transparent outline-none text-xs text-paper-100 placeholder:text-ink-muted" /><button onClick={toggleVoice} aria-label={recording ? 'Stop voice input' : 'Voice input'} className={`p-1.5 rounded-lg ${recording ? 'bg-accent-subtle text-accent' : 'hover:bg-canvas-elevated text-ink-muted'}`}><Mic className="w-4 h-4" /></button></div>{showCommands && <div className="mt-2 space-y-1">{filteredCommands.slice(0, 6).map(([key, label, action]) => <button key={key} onClick={() => chooseCommand(action)} className="w-full flex items-center justify-between rounded-lg px-2.5 py-2 text-left hover:bg-canvas-elevated"><span><span className="block text-[11px] text-paper-100">{label}</span><span className="block text-[9px] text-ink-muted">{key}</span></span><ChevronRight className="w-3 h-3 text-ink-muted" /></button>)}</div>}</section>

            <section className="rounded-xl border border-border bg-canvas-base/50 p-3"><div className="flex items-center justify-between mb-2"><div className="flex items-center gap-2 text-xs font-semibold text-paper-100"><Activity className="w-3.5 h-3.5 text-accent" /> Live work</div><span className="text-[9px] text-ink-muted">{activity.length ? `${activity.length} events` : 'Waiting'}</span></div>{activity.length ? <div className="space-y-1.5">{activity.slice(0, 6).map((item) => <div key={item.id} className="flex items-center gap-2 text-[10px]">{item.status === 'running' ? <Loader2 className="w-3.5 h-3.5 animate-spin text-accent" /> : <Check className="w-3.5 h-3.5 text-signal-success" />}<span className="flex-1 truncate text-paper-200">{item.label}</span><span className="text-ink-muted">{item.status}</span></div>)}</div> : <div className="text-[10px] text-ink-muted">Thinking · Reading · Searching · Writing · Executing · Verifying</div>}</section>

            <section className="rounded-xl border border-border bg-canvas-base/50 p-3"><div className="flex items-center justify-between mb-2"><div className="flex items-center gap-2 text-xs font-semibold text-paper-100"><Paperclip className="w-3.5 h-3.5 text-accent" /> Multi-file context</div><span className="text-[9px] text-ink-muted">{files.length}/{MAX_FILES}</span></div><input ref={pickerRef} type="file" multiple className="hidden" accept="image/*,.pdf,.txt,.md,.json,.csv,.js,.jsx,.ts,.tsx,.py,.html,.css,.java,.c,.cpp,.go,.rs,.xlsx,.docx,.pptx" onChange={onFilesSelected} /><div className="flex gap-2"><button onClick={() => pickerRef.current?.click()} className="flex-1 flex items-center justify-center gap-1.5 px-3 py-2 rounded-lg border border-border bg-canvas-surface text-[10px] text-paper-200 hover:bg-canvas-elevated"><Upload className="w-3.5 h-3.5" /> Add files</button><button disabled={!files.length || batchRunning} onClick={analyzeBatch} className="px-3 py-2 rounded-lg bg-accent text-black text-[10px] font-semibold disabled:opacity-40">{batchRunning ? `${batchProgress}%` : 'Analyze'}</button></div>{files.length > 0 && <div className="grid grid-cols-2 gap-1.5 mt-2">{files.map((file) => <button key={file.id} onClick={() => { setActiveId(file.id); setSplitOpen(true); }} className={`rounded-lg border p-2 text-left ${activeId === file.id ? 'border-accent/60 bg-accent-subtle' : 'border-border bg-canvas-surface'}`}><div className="flex items-center gap-1.5 min-w-0">{file.kind === 'image' ? <ImageIcon className="w-3.5 h-3.5 text-accent" /> : <FileText className="w-3.5 h-3.5 text-ink-muted" />}<span className="text-[10px] truncate text-paper-200 flex-1">{file.name}</span><span onClick={(event) => { event.stopPropagation(); removeFile(file.id); }} className="text-ink-muted hover:text-paper-100"><X className="w-3 h-3" /></span></div></button>)}</div>}{batchResults.length > 0 && <div className="mt-2 space-y-1.5"><div className="text-[9px] uppercase tracking-wider text-ink-muted">Batch analysis</div>{batchResults.map((item) => <div key={item.file} className="text-[10px] text-paper-200"><strong>{item.file}</strong>: {item.success ? 'done' : 'failed'}</div>)}<button onClick={continueInChat} className="mt-1 text-[10px] text-accent hover:underline">Continue in main chat with these results →</button></div>}</section>
